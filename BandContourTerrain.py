...
class colorCallback(object):
	...
	def slide(self, obj, event):
		colorRep = obj.GetRepresentation()
		val = colorRep.GetValue()
		self.actor.GetProperty().SetColor(val, val, val)
...
lutWater.SetNumberOfColors(5)
lutWater.SetHueRange(0.15, 0.99)
lutWater.SetSaturationRange(0.5, 0.9)
lutWater.SetValueRange(0.1, 1.0)
lutWater.Build()
lutLand = vtk.vtkLookupTable()
lutLand.SetNumberOfColors(5)
lutLand.SetHueRange(0.1, 0.9)
lutLand.SetSaturationRange(0.1, 0.7)
lutLand.SetValueRange(0.15, 1.0)
lutLand.Build()
